...
mock.onGet('/api/articles').reply(() => {
  return [200, JSON.parse(JSON.stringify(data.articles))]
})

mock.onGet('/api/categories').reply(() => {
  return [200, JSON.parse(JSON.stringify(data.categories))]
})

mock.onGet('/api/articles/top').reply(() => {
  const article = _.find(data.articles, { id: 1 })

  return [200, JSON.parse(JSON.stringify(article))]
})

mock.onGet('/api/articles/recent').reply(() => {
  const recent = _.filter(data.articles, art => art.id > 1)

  return [200, JSON.parse(JSON.stringify(recent))]
})

mock.onGet(/\/api\/articles\/category\/\d+/).reply((request) => {
  const itemId = request.url.substring(request.url.lastIndexOf('/') + 1)
  const item = _.filter(data.articles, art => art.category === Number(itemId))

  return [200, JSON.parse(JSON.stringify(item))]
})

mock.onGet(/^\/api\/articles\/(.+)/).reply((request) => {
  const slug = request.url.substring(request.url.lastIndexOf('/') + 1)
  const article = _.find(data.articles, { slug })
